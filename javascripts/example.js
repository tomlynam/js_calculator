...
function numInput(num) {
    ...
    // This is to make sure the first number isn't 0 because then 
    // javascript thinks we are using OCTAL (Base eight)
    if ((display.value == "") && num == "0") {
    // If it is, do nothing
      return;
    }
    // Check if a calculation has finished
    // If it has replace the number in the display 
    // (the answer to the calculation with the number
...
function setOperation(command) {
    ...
    // Queue up an operation for enterint multiple 
    // commands without hitting equals
    // i.e. 10x4+8-9+3=
